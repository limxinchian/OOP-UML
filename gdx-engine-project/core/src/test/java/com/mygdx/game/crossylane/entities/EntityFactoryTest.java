...
public class EntityFactoryTest {
    ...
    @Test
    public void createGoalZoneUsesConfiguredBounds() {
        GoalZoneEntity goalZone = EntityFactory.createGoalZone();
        TransformComponent transform = goalZone.getComponent(TransformComponent.class);

        assertNotNull(transform);
        assertEquals(0f, transform.getPositionX(), 0.0001f);
        assertEquals(CrossyLaneConfig.GOAL_ZONE_Y, transform.getPositionY(), 0.0001f);
        assertEquals(CrossyLaneConfig.WORLD_WIDTH, transform.getWidth(), 0.0001f);
        assertEquals(CrossyLaneConfig.GOAL_ZONE_HEIGHT, transform.getHeight(), 0.0001f);
    }
    ...
    public void createLaneMarkersCreatesOneMarkerPerLaneBoundary() {
        List<LaneMarkerEntity> markers = EntityFactory.createLaneMarkers(4);
