...
public class SettingsScene implements IScene<CrossyLaneSceneKey> {
    ...
    public SettingsScene(SceneManager<CrossyLaneSceneKey> sceneManager,
                    IOManager ioManager,
    ...
    public void update(float delta) {
        ...
        float panelWidth = MathUtil.clamp(screenW * 0.42f, 560f, 760f);
        float rowHeight = MathUtil.clamp(screenH * 0.08f, 56f, 74f);
        float rowGap = MathUtil.clamp(screenH * 0.02f, 16f, 22f);

        float titleHeight = MathUtil.clamp(screenH * 0.09f, 74f, 92f);
        float hintBoxHeight = MathUtil.clamp(screenH * 0.05f, 42f, 52f);
    ...
    private void adjustSelected(float amount) {
        if (selectedRow == 0) {
            musicVolume = snapVolume(MathUtil.clamp(musicVolume + amount, 0f, 1f));
            audioController.setMusicVolume(musicVolume);
        } else if (selectedRow == 1) {
            sfxVolume = snapVolume(MathUtil.clamp(sfxVolume + amount, 0f, 1f));
            audioController.setSfxVolume(sfxVolume);
        }
    }
    ...
    public void render() {
        ...
        float titleWidth = MathUtil.clamp(screenW * 0.22f, 280f, 420f);
        float titleHeight = MathUtil.clamp(screenH * 0.09f, 74f, 92f);
        float hintBoxHeight = MathUtil.clamp(screenH * 0.05f, 42f, 52f);
