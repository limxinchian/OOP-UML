...
public class TrafficLightController {
    ...
    /**
     * Returns the score delta when the player transitions between lane positions.
     *
     * Scoring triggers when the player steps from off-road (grass/goal) onto
     * ANY road lane.  The traffic light is global — it governs the entire road,
     * not a single lane.  Moving between road lanes does not re-trigger scoring;
     * only a fresh grass → road entry counts.
     *
     * Red  = penalty (negative delta).
     * Green = safe, no penalty and no bonus (returns 0 via greenLaneEntryScoreDelta).
     */
    public int scoreForLaneEntry(int previousLaneIndex, int currentLaneIndex) {
        if (!isEnteringRoadFromOffRoad(previousLaneIndex, currentLaneIndex)) {
            return 0;
        }
    ...
    /**
     * Returns true when the player transitions from off-road (NO_LANE_INDEX)
     * onto any valid road lane (>= 0).
     */
    private boolean isEnteringRoadFromOffRoad(int previousLaneIndex, int currentLaneIndex) {
