...
import com.mygdx.game.crossylane.entity.SafeStopZoneEntity;
import com.mygdx.game.crossylane.entity.TrafficLightEntity;
import com.mygdx.game.crossylane.entity.ZebraCrossingEntity;
...
public class EntityFactory {
    ...
    public static LaneMarkerEntity createLaneMarker(LaneLayout laneLayout, int boundaryIndex) {
        float markerY = laneLayout.getLaneBaseY(boundaryIndex)
                - CrossyLaneConfig.LANE_MARKER_HEIGHT / 2f;
        return new LaneMarkerEntity(markerY);
    }

    public static List<LaneMarkerEntity> createLaneMarkers(LaneLayout laneLayout) {
        List<LaneMarkerEntity> markers = new ArrayList<>();
        for (int i = 1; i < laneLayout.getLaneCount(); i++) {
            markers.add(createLaneMarker(laneLayout, i));
        }
        return markers;
    }
