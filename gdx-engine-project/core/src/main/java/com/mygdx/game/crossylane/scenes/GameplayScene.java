...
public class GameplayScene implements IScene<CrossyLaneSceneKey> {
    ...
    private void renderEntitiesManually() {
        for (Entity entity : entityManager.getEntities()) {
            ...
            if (transform == null || renderable == null || !renderable.isEnabled()) {
                continue;
            }

            shapeRenderer.setColor(renderable.r(), renderable.g(), renderable.b(), renderable.a());

            if (renderable.getShape() == RenderShape.RECTANGLE) {
                shapeRenderer.rect(
                        transform.getPositionX(),
                        transform.getPositionY(),
                        transform.getWidth(),
                        transform.getHeight());
            } else if (renderable.getShape() == RenderShape.CIRCLE) {
                float radius = renderable.getRadius();
                shapeRenderer.circle(
                        transform.getPositionX() + radius,
                        transform.getPositionY() + radius,
                        radius);
            }
        }
    }
